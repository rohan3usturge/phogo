...
class DBProvider {
  ...
  initDB() async {
    ...
    String path = join(documentsDir.path, 'phogo.db');
  ...
  newNote(ImageBin imagebin) async {
  ...
  getNotes() async {
    ...
    List<ImageBin> notes = res.isNotEmpty
        ? res.map((note) => ImageBin.fromJson(note)).toList()
        : [];
